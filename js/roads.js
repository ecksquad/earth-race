  ...
  async _loadTile(tx, ty, key) {
    ...
    if (!raw) {
      try {
        ...
        const ql = `[out:json][timeout:25];(way["highway"](${b});way["amenity"="parking"](${b}););out geom;`;
    ...
    // Nitro pickups aren't a real OSM feature — scattered at random along the
    // segments this tile actually has, at load time, so they're stable for
    // the rest of the race but different next time you drive the same area.
    // Not seeded/deterministic like the 1000 fixed collectables — these are
    // meant to vary race to race, so they're never cached above.
    const nitros = [];
    for (const s of segs) {
      if (Math.random() < NITRO_CHANCE) {
        nitros.push({ x: (s.x1 + s.x2) / 2, y: (s.y1 + s.y2) / 2, collectedAt: 0 });
      }
    }
